...
export const VideoGrid: React.FC<VideoGridProps> = ({
  ...
  return (
    <div className="vg-container">
      ...
      <style>{`
        .vg-container { position: relative; width: 100%; border-radius: var(--radius-lg); overflow: hidden; background: #000; aspect-ratio: 16/9; animation: scaleIn 0.3s ease; }
        ...
        .vg-status-bar { position: absolute; top: 12px; left: 12px; display: flex; align-items: center; gap: 0.5rem; background: rgba(0,0,0,0.6); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); color: #fff; padding: 0.4rem 0.8rem; border-radius: var(--radius-full); font-size: 0.8rem; font-weight: 600; z-index: 2; animation: slideDown 0.3s ease; }
        .vg-pip-wrap { position: absolute; bottom: 16px; right: 16px; width: 220px; height: 165px; border-radius: var(--radius-lg); overflow: hidden; border: 2px solid rgba(255,255,255,0.4); box-shadow: 0 4px 20px rgba(0,0,0,0.5); z-index: 3; cursor: grab; animation: scaleIn 0.3s ease 0.1s both; }
        .vg-pip-video { width: 100%; height: 100%; object-fit: cover; }

        @media (max-width: 1024px) {
          .vg-container { border-radius: 0; aspect-ratio: auto; flex: 1; height: calc(100dvh - 220px); min-height: 260px; max-height: calc(100dvh - 140px); }
          .vg-pip-wrap { width: 130px; height: 97px; bottom: 12px; right: 12px; border-radius: var(--radius-md); }
          .vg-status-bar { top: 8px; left: 8px; font-size: 0.75rem; padding: 0.3rem 0.6rem; }
        }

        @media (max-width: 480px) {
          .vg-container { border-radius: 0; height: calc(100dvh - 200px); min-height: 220px; }
